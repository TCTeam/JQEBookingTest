$(this).ready(function(e) {
	...
	$("#Comments").click(function() {
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=CommentsQuery"),
			type: "get",
	...
	$(".Item6QueryTime").click(function() {
		...
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=CommentsQuery&QueryTime=" + queryTime),
			type: "get",
			success: function(data) {
				tempdata = data;
				$("#Item6Table tr:not('.tabtitle')").remove();
	...
	$("#Item6CustomQuery").click(function() {
		...
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=CommentsQuery&CustomQuery=CustomQuery&StartDate=" + startDate + "&EndDate=" + endDate + "&Evaluate=" + Evaluate),
			type: "get",
			success: function(data) {
				tempdata = data;
				$("#Item6Table tr:not('.tabtitle')").remove();
	...
	$("#BtnCommentsReply").click(function() {
		...
		if (commentsReply == "") {
			...
			$.ajax({
				url: encodeURI("AccessDataWithAjax.aspx?type=CommentsReply&orderSerialNo=" + orderSerialNo + "&commentsReply=" + commentsReply),
				type: "get",
				success: function(data) {
					if (data == "true") {
						alert("回复成功");
	...
	$("#OrderQuery").click(function() {
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=OrderQuery"),
			type: "get",
			success: function(data) {
	...
	$("#ButtonItem2Query").click(function() {
		...
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=OrderQuery&orderComfirmNo=" + orderComfirmNo + "&takeTicketTime=" + takeTicketTime + "&orderNo=" + orderNo + "&identityId=" + identityId + "&takeTicketName=" + takeTicketName + "&takeTicketPhone=" + takeTicketPhone + "&ticketSort=" + ticketSort + "&sortType=" + sortType + "&payType=" + payType),
			type: "get",
			success: function(data) {
				tempdata = data;
				$("#TabelOrderQuery tr:not('.tabtitle')").remove();
	...
	$("#OrderConfirm").click(function() {
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=OrderConfirmQuery"),
			type: "get",
			success: function(data) {
				tempdata = data;
	...
	$("#ButtonItem3Query").click(function() {
		...
		var ticketSort = $("#Item3TicketSort option:selected").val();
		var payType = $("input:radio[name='Item3payType']:checked").val();
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=OrderConfirmQuery&chooseDate=" + chooseDate + "&dateStart=" + dateStart + "&dateEnd=" + dateEnd + "&otOrderSerialNo=" + orderSerialNum + "&orderConfirmNum=" + orderConfirmNum + "&orderName=" + orderName + "&ticketPhone=" + ticketPhone + "&ticketSort=" + ticketSort + "&payType=" + payType),
			type: "get",
			success: function(data) {
				tempdata = data;
				$("#TableOrderConfirm tr:not('.tabtitle')").remove();
	...
	$("#ButtonItem4Query").click(function() {
		...
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=OrderStatistical&chooseOrder=" + chooseOrder + "&dateStart1=" + dateStart1 + "&dateEnd1=" + dateEnd1 + "&dateStart2=" + dateStart2 + "&dateEnd2=" + dateEnd2 + "&conStion=" + conStion),
			type: "get",
			success: function(data) {
				tempdata = data;
	...
	$("#OrderSta").click(function() {
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?index=1&type=OrderStatistical"),
			type: "get",
			success: function(data) {
				tempdata = data;
				$("#Chitem4 tr:not('.tabtitle')").remove();
	...
	$("#ButtonConfirm").click(function() {
		...
		$.ajax({
			url: encodeURI("AccessDataWithAjax.aspx?type=OrderConfirmOk&otOrderSerialNo=" + otOrderSerialNo + "&realTicketNum=" + realTicketNum),
			type: "get",
			success: function(data) {
				if (data == "true") {
					alert("订单确认成功");
	...
	$("#AddBz").click(function() {
        ...
        if (text === $(this).attr("data-text")) {
			...
			$.ajax({
				url: encodeURI("AccessDataWithAjax.aspx?index=&type=UpdataRemark&otOrderSerialNo=" + otOrderSerialNo + "&text=" + text),
				type: "get",
				success: function(data) {
					if (data === "true") {
						$("#body_cont").hide();
	...
	function OrderConfirmInit() {
		...
		$(".CancelReConfirm").click(function() {
			if (confirm("您确定游客未购票吗？")) {
				var otOrderSerialNo = $(this).attr("data");
				$.ajax({
					url: encodeURI("AccessDataWithAjax.aspx?type=CancelReConfirm&otOrderSerialNo=" + otOrderSerialNo),
					type: "get",
					success: function(data) {
						if (data == "true") {
							alert("操作成功");
		...
		$(".sbutton").click(function() {
			...
			var otOrderSerialNo = $(this).attr("data");
			$.ajax({
				url: encodeURI("AccessDataWithAjax.aspx?type=GetRemark&otOrderSerialNo=" + otOrderSerialNo + "&randomtext=" + Math.random()),
				type: "get",
				success: function(data) {
					if (data != "") {
						$(".BzText").val(data);
					...
					$("#AddBz").attr("data", otOrderSerialNo);
					$("#AddBz").attr("data-text", textarea);
				}
			});
			return;
		});
		...
		$(".item2ListPage").click(function() {
			$.ajax({
				url: encodeURI("AccessDataWithAjax.aspx?index=" + $(this).text() + "&type=OrderQuery"),
				type: "get",
		...
		$(".item3ListPage").click(function() {
			$.ajax({
				url: encodeURI("AccessDataWithAjax.aspx?index=" + $(this).text() + "&type=OrderConfirmQuery"),
				type: "get",
                contentType:"application/x-www-form-urlencoded;charset=GB2312",
				success: function(data) {
					$("#TableOrderConfirm tr:not('.tabtitle')").remove();
		...
		$(".item4ListPage").click(function() {
			$.ajax({
				url: encodeURI("AccessDataWithAjax.aspx?index=" + $(this).text() + "&type=OrderStatistical"),
				type: "get",
				success: function(data) {
					$("#Chitem4 tr:not('.tabtitle')").remove();
					$("#Chitem4").append(data);
		...
		$(".item6ListPage").click(function() {
			$.ajax({
				url: encodeURI("AccessDataWithAjax.aspx?index=" + $(this).text() + "&type=CommentsQuery"),
				type: "get",
				success: function(data) {
					$("#Item6Table tr:not('.tabtitle')").remove();
					$("#Item6Table").append(data);
